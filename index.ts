...
async function analyze_message(message: Prisma.MessageCreateInput) {
  try {
    ...
    if (category == Type.discuss) {
      ...
      send_message({ ...response_message, content: openAIResponse.data.choices[0].text, response_time: message.response_time })
      ...
      await send_message({ ...response_message, content: `${response[0].toLowerCase().replace(/\s/g, '') == 'view' ? 'your bio:' : 'updated bio:'}\n${response[1]}` })
      ...
      const response = openAIResponse.data.choices[0].text
      await send_message({ ...response_message, content: response ? response : 'Sorry bugged out. Try again' })
    } else if (category == Type.quote) {
      await send_message({ ...response_message, content: get_quote() })
    } else if (category == Type.customer_support) {
      send_message({ ...response_message, content: `Sorry for the inconvenience, somebody from the team will reach out.` })
      send_message({ ...response_message, content: ` ! Customer support request from (${user.number})\n${message.content}` }, admins)
...
async function get_previous_messages(message: Prisma.MessageCreateInput, amount: number = 14, chat: boolean) {
...
async function send_message(message: Prisma.MessageCreateInput, users?: User[]) {
  try {
