...
(function (AdminNumbers) {
    AdminNumbers["Watts"] = "+13104974985";
    AdminNumbers["Pulice"] = "+12015190240";
})(AdminNumbers || (AdminNumbers = {}));
const signup_link = 'https://tally.so/r/w4Q7kX', admin_numbers = Object.values(AdminNumbers);
// ======================================================================================
// ========================================ROUTES========================================
// ======================================================================================
app.post('/signup-form', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!users.includes(user.number)) {
            yield send_message(Object.assign(Object.assign({}, default_message), { content: greeting_message.content, number: user.number, media_url: greeting_message.media_url, send_style: greeting_message.send_style, response_time: t0 }));
...
const job = new cron_1.default.CronJob('55 */1 * * *', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    console.log(`t0: ${t0}`);
    const users_with_timezone = yield prisma.user.findMany({ where: { timezone: { not: null } } });
    users_with_timezone.forEach((user) => __awaiter(void 0, void 0, void 0, function* () {
        if (times.includes(t0 + timezones.indexOf(user.timezone))) {
            yield send_message(Object.assign(Object.assign({}, default_message), { content: quotes[Math.floor(Math.random() * quotes.length)], number: user.number, response_time: t0 }));
        }
    }));
}));
job.start();
...
function local_data() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            users = yield prisma.user.findMany().then(users => users.map(user => user.number));
            id = yield prisma.message.findFirst({ orderBy: { id: 'desc' } }).then(message => message.id);
        }
        catch (e) {
            console.log(e);
        }
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (message.content.toLowerCase().includes('admin\n...') && admin_numbers.includes(message.number)) {
                yield send_message(Object.assign(Object.assign({}, default_message), { content: message.content.split('...\n').pop(), type: client_1.Type.question }), users);
                return;
            } // ADJUST WEIGHTS
                ...
                if (values == 'reset')
                    temp = null, pres = null, freq = null;
                else
                    temp = weights[0], pres = weights[1], freq = weights[2];
                yield sendblue.sendMessage({ content: `weights updated from (temp,pres,freq) = (${user.temp},${user.pres},${user.freq}) to (${temp},${pres},${freq})`, number: message.number, status_callback: sendblue_callback }); // don't log the message
                yield prisma.user.update({ where: { number: message.number }, data: { temp, pres, freq } });
                return;
            } // ADJUST MODEL
                ...
                if (model == 'chat' || model == 'text' || model == 'rapid') {
                    ...
                    yield sendblue.sendMessage({ content: `${model} activated\nweights (temp,pres,freq) = ${user.temp}, ${user.pres}, ${user.freq}\ndefault weights = ${temp}, ${pres}, ${freq}`, number: message.number, status_callback: sendblue_callback });
                    return;
                }
                yield sendblue.sendMessage({ content: `think your formatting's wrong, try 'm:chat', 'm:text', 'm:rapid'`, number: message.number, status_callback: sendblue_callback });
                return;
            }
      ...
      Category:`
            ...
            if (category == client_1.Type.discuss) {
                ...
                send_message(Object.assign(Object.assign({}, default_message), { content: openAIResponse.data.choices[0].text, number: message.number, tokens: message.tokens, response_time: message.response_time }));
            }
            else if (category == client_1.Type.update_profile) {
                const user = yield prisma.user.findFirst({ where: { number: message.number } });
                let openAIResponse = yield openai.createCompletion({
                ...
                yield send_message(Object.assign(Object.assign({}, default_message), { content: `${response[0].toLowerCase().replace(/\s/g, '') == 'view' ? 'your bio:' : 'updated bio:'}\n${response[1]}`, number: message.number }));
                ...
                yield send_message(Object.assign(Object.assign({}, default_message), { content: response ? response : 'Sorry bugged out. Try again', number: message.number }));
            }
...
function get_previous_messages(message, amount = 14, chat) {
    return __awaiter(this, void 0, void 0, function* () {
        const resetMessage = yield prisma.message.findFirst({ where: { number: message.number, content: 'reset' }, orderBy: { id: 'desc' } }); // TODO not ideal cuz parses EVERY message from that number lol
        let resetMessageLoc;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (message.response_time)
                message.response_time = Number(t0.valueOf() - message.response_time);
...
function error_alert(error, message) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (message)
            send_message(Object.assign(Object.assign({}, default_message), { content: `Sorry bugged out, try again.`, number: message.number }));
    });
}
const greeting_message = Object.assign(Object.assign({}, default_message), { content: `Hi I'm jrnl, your conversational AI journal. We're trying to  Reply to my questions or text me when you want. I messsage every 3 hours throughout the day, Feel free to react to messages to better train me. Everything operates on natural language, so no need to learn any fancy commands. Your 'bio' provides me insight, helping me help you. Ask to view it or change it anytime. Remember, no special commands just speak as you would Add the contact card and pin me in your messages for quick access.`, media_url: `${link}/assets/jrnl.vcf`, send_style: client_1.SendStyle.lasers });
// ======================================================================================
// ========================================TESTING=======================================
// ======================================================================================
// test()
