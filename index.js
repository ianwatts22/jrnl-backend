...
app.post('/signup-form', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        res.status(200).end();
        let user = { number: fields[0].value, name: fields[1].value, gender: fields[2].value, birthdate: new Date(fields[8].value), location: fields[9].value }; // Tally webhooks data formatting is a nightmare
        let existingUser = yield prisma.users.findFirst({ where: { number: user.number } });
        if (!existingUser) {
...
const getUser = (message) => __awaiter(void 0, void 0, void 0, function* () { return yield prisma.users.findFirst({ where: { number: message.number }, }); });
